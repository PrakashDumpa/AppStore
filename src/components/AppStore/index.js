...
class AppStore extends Component {
  state = {isActiveId: tabsList[0].tabId, isSearchInput: ''}
  ...
  changeSearchInput = event => {
    this.setState({isSearchInput: event.target.value.toLowerCase()})
  }

  render() {
    const {isSearchInput, isActiveId} = this.state
    let filteredAppsList = this.getFilteredAppsList()
    filteredAppsList = filteredAppsList.filter(each =>
      each.appName.toLowerCase().includes(isSearchInput),
    )
    ...
    return (
      <div className="bg_container">
        <div className="main_container mt-5 mb-5">
          ...
          <ul className="list-unstyled tabs_container">
            {tabsList.map(eachTabListDetails => (
              <TabItem
                tabsListItems={eachTabListDetails}
                key={eachTabListDetails.tabId}
                updateActivatedId={this.updateActivatedId}
                Active={eachTabListDetails.tabId === isActiveId}
              />
            ))}
          </ul>
          <ul className="list-unstyled apps_container">
            {filteredAppsList.length < 1 ? (
              <div className="not_found">
                <h5>No Result Found</h5>
              </div>
            ) : (
              filteredAppsList.map(eachAppItemDetails => (
                <AppItem
                  appsItemList={eachAppItemDetails}
